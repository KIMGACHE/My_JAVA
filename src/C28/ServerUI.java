...
class Sgui extends JFrame implements ActionListener,KeyListener
{
	...
	Sgui() throws Exception{
		...
		client = server.accept(); // 받기전까지 해당 라인에서 멈춘다.
		
		// 수신 스레드(스레드 분리, 따로작업)
		ServerRecvThread recv = new ServerRecvThread(this, client); // recv가 client로부터 정보를 받아야 하므로 client를 넣어주고 Sgui에 받은 정보를 넘겨줘야하므로 Sgui의 위치에 대해 알기위해 this를 받는다.
		Thread th = new Thread(recv);
		th.start();
		
		// 송신 스레드(==메인스레드 내의 함수로 처리)
