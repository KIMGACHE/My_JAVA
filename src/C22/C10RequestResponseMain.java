...
// maven repository에서 jsoup, jackson databind, jactson core, jackson annotation
// 최신버전 드가서 jar다운로드 - 그걸 해당 패키지에 드래그 앤 드롭으로 넣고 project우클릭 후 property - java build path - library - add jar해서 가져온 파일 넣고 apply
...
public class C10RequestResponseMain {
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		// Http Request 구조화
		HttpRequest httpRequest = HttpRequest.newBuilder() // HTTP객체를 만드는 과정
									.uri(URI.create(url)) // 어느 위치로 요청할건지
									.GET() // 어떤 방식으로 요쳥할건지
									.build(); 
		// Http 요청
		HttpClient httpClient = HttpClient.newHttpClient(); // browser인 척하는 녀석
		HttpResponse<String> response = httpClient.send(httpRequest, HttpResponse.BodyHandlers.ofString()); // (요청객체,응답이왔을때 어떤식으로 저장되게끔 할것인가-문자열로)
//		System.out.println(response.body());
		
		ObjectMapper objectMapper = new ObjectMapper(); 
		
		// java class로 변환하는 방법 - json to java라고 치면 여러개나오는데 아무대나 가서 바꾸기
//		Root responseData = objectMapper.readValue(response.body(), Root.class); // body안에 있는 내용을 Root?
...
//		System.out.println("data[2] : " + responseData.getData().get(2));
	...
	private static class Root{
		...
		@Override
		public String toString() {
			return "Root [status=" + status + ", total=" + total + ", data=" + data + "]";
		}
