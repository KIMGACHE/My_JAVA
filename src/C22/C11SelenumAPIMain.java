...
public class C11SelenumAPIMain {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		WebElement searchEl = driver.findElement(By.id("query")); // 드라이버로 연결해놓은 브라우저에서 element를 찾는다. query라는 선택자

		//키워드 입력
		searchEl.sendKeys("노트북"); // 여기에 적는 값을 입력하여 검색을 자동으로 시켜줌
		
		//엔터키 전달
		searchEl.sendKeys(Keys.RETURN);
		
		//노트북 검색 이후에 쇼핑 버튼 클릭
		WebElement shoppingBtnEl =  driver.findElement(By.cssSelector(".api_pcpg_wrap .tab:nth-child(1)")); // 해당 선택자를 찾아서 클릭하고있음
		
		//target_blank(새창열기) 제거 
		JavascriptExecutor js = (JavascriptExecutor)driver;
		js.executeScript("arguments[0].removeAttribute('target')",shoppingBtnEl); // 새탭이 열리지않고 현재 창에서 진행함
		shoppingBtnEl.click();
