...
class GUI extends JFrame implements ActionListener,KeyListener{
	...
	GUI(){
		//Frame
		super("프레임창입니다");
		setBounds(100,100,350,400); // 모니터 기준 가로세로
...
//		panel.add(area1);
		...
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // 기본X버튼 생성
		setVisible(true);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==btn1)	//저장하기(FileWriter) 
		{
			...
			try {
				String filename = UUID.randomUUID().toString(); // UUID를 통해 랜덤으로 파일이름 설정
				Writer out = new FileWriter(DIR+filename+".txt");
				String contents = area1.getText(); // area에 있는 모든 텍스트를 가져온다.
				out.write(contents);
				out.flush();
				out.close();
		...
		{
			System.out.println("불러오기 CLICKED"); // 불러오려면 일단 파일을 선택해야 함
			JFileChooser fileChooser = new JFileChooser(); // 파일 탐색기 비슷한 것
			File defaultDirPath = new File(DIR);
			fileChooser.setCurrentDirectory(defaultDirPath); // 파일 탐색기의 기본 경로를 DIR로 설정
			int val = fileChooser.showOpenDialog(null); // val을 통해 특정 파일을 선택했는지 확인할 수 있다.
			if(val==JFileChooser.APPROVE_OPTION) { // APPROVE_OPTION이라는 상수와 같다면
				String filename = fileChooser.getSelectedFile().toString(); // 단일 파일이면 그냥 getSelectedFile 복수면 s
