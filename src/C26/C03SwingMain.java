...
class C03GUI extends JFrame implements ActionListener, KeyListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btn1) {
			...
			if(th1==null) {
				C03Worker1 w1 = new C03Worker1(this); // Worker1객체를 만들 때 Swing객체의 주소를 전달한다.
				th1 = new Thread(w1);
				th1.start();
			}
		...
		if (e.getSource() == btn2) {
			...
			if(th1!=null) {
				th1.interrupt(); // thread를 중단시키기 위해,interrupt객체=예외가 만들어진다.
				th1=null;
			}
		...
		if (e.getSource() == btn4) {
			...
			if(th2!=null) {
				th2.interrupt(); // thread를 중단시키기 위해,interrupt객체=예외가 만들어진다.
				th2=null;
			}
